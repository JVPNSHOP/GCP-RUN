...
LOG_FILE="/tmp/pussy1990_vless_$(date +%s).log"
touch "$LOG_FILE"
on_err() {
  local rc=$?
  echo "" | tee -a "$LOG_FILE"
...
# =================== Banner (image-style 404 replaced by @Pussy1990) ===================
show_banner() {
  clear
  printf "\n\n"
  printf "${C_RED}${BOLD}"
  printf "╔══════════════════════════════════════════════════════════════════╗\n"
  printf "║    ${C_CYAN} ___   ___          ________          ___   ___                               ${C_RED}\n"
  printf "║    ${C_CYAN}|\\  \\ |\\  \\        |\\   __  \\        |\\  \\ |\\  \\                              ${C_RED}\n"
  printf "║    ${C_CYAN}\\\\ \\  \\|_\\\\  \\       \\\\ \\  \\|\\  \\       \\\\ \\  \\|_\\\\  \\                             ${C_RED}\n"
  printf "║    ${C_CYAN} \\ \\______  \\       \\ \\  \\/\\  \\       \\ \\______  \\                            ${C_RED}\n"
  printf "║    ${C_CYAN}  \\|_____|\  \\       \\ \\  \\/\\  \\       \\|_____|\  \\                           ${C_RED}\n"
  printf "║    ${C_CYAN}         \\ \\__\\       \\ \\_______\\             \\ \\__\\                          ${C_RED}\n"
  printf "║    ${C_CYAN}          \\|__|        \\|_______|              \\|__|                          ${C_RED}\n"
  printf "║                                                                                               ${C_RED}\n"
  printf "║         ${C_YELLOW}🚀 VLESS WS DEPLOYMENT SYSTEM => VERSION - 2.0                         ${C_RED}\n"
  printf "║         ${C_GREEN}⚡ Powered by @Pussy1990                                               ${C_RED}\n"
  printf "║                                                                                               ${C_RED}\n"
  printf "╚══════════════════════════════════════════════════════════════════╝${RESET}\n"
  printf "\n"
  # The 3-block isometric 404-like graphic (kept similar style) but label below uses @Pussy1990
  printf "${C_CYAN}"
  printf "    ___       ___        ___\n"
  printf "   /__/|     /__/|      /__/|\n"
  printf "  |  |:|    |  |:|     |  |:|\n"
  ...
  printf "  |__|/     |__|/      |__|/\n"
  printf "${RESET}\n\n"
  # Scripts By : Jue Htet in three colors
  printf "    ${C_CYAN}Scripts By:${RESET} ${C_YELLOW}Jue${RESET} ${C_GREEN}Htet${RESET}\n\n"
  printf "${C_BLUE}${BOLD}    @Pussy1990${RESET}\n\n"
}
...
show_success() { printf "${C_GREEN}${BOLD}✓${RESET} ${C_GREEN}%s${RESET}\n" "$1"; }
show_info()    { printf "${C_BLUE}${BOLD}ℹ${RESET} ${C_BLUE}%s${RESET}\n" "$1"; }
show_warning() { printf "${C_YELLOW}${BOLD}⚠${RESET} ${C_YELLOW}%s${RESET}\n" "$1"; }
show_error()   { printf "${C_RED}${BOLD}✗${RESET} ${C_RED}%s${RESET}\n" "$1"; }
show_divider() { printf "${C_GRAY}%s${RESET}\n" "──────────────────────────────────────────────────────────"; }
show_kv()      { printf "   ${C_GRAY}%s${RESET}  ${C_CYAN}%s${RESET}\n" "$1" "$2"; }
...
printf "\n${C_YELLOW}┌──────────────────────────────────────────────────────┐${RESET}\n"
printf "${C_YELLOW}│${RESET} ${C_CYAN}🔑 Telegram Bot Configuration${RESET}                      ${C_YELLOW}│${RESET}\n"
printf "${C_YELLOW}└──────────────────────────────────────────────────────┘${RESET}\n\n"

read -rp "${C_GREEN}🤖 Enter Telegram Bot Token:${RESET} " _tk || true
...
read -rp "${C_GREEN}👤 Enter Owner/Channel Chat ID(s):${RESET} " _ids || true
[[ -n "${_ids:-}" ]] && TELEGRAM_CHAT_IDS="${_ids// /}"

DEFAULT_LABEL="Join @Pussy1990"
DEFAULT_URL="https://t.me/premium_channel_404"
BTN_LABELS=(); BTN_URLS=()

printf "\n${C_YELLOW}┌──────────────────────────────────────────────────────┐${RESET}\n"
printf "${C_YELLOW}│${RESET} ${C_CYAN}🔘 Inline Button Configuration (Optional)${RESET}            ${C_YELLOW}│${RESET}\n"
printf "${C_YELLOW}└──────────────────────────────────────────────────────┘${RESET}\n\n"

read -rp "${C_GREEN}➕ Add URL button(s)? [y/N]:${RESET} " _addbtn || true
if [[ "${_addbtn:-}" =~ ^([yY]|yes)$ ]]; then
  i=0
  while true; do
    printf "\n${C_GRAY}── Button $((i+1)) ──${RESET}\n"
    read -rp "${C_GREEN}🔖 Label [default: ${DEFAULT_LABEL}]:${RESET} " _lbl || true
    if [[ -z "${_lbl:-}" ]]; then
      BTN_LABELS+=("${DEFAULT_LABEL}")
      BTN_URLS+=("${DEFAULT_URL}")
      show_success "Added: ${DEFAULT_LABEL} → ${DEFAULT_URL}"
    else
      read -rp "${C_GREEN}🔗 URL (http/https):${RESET} " _url || true
    ...
    i=$(( i + 1 ))
    (( i >= 3 )) && break
    read -rp "${C_GREEN}➕ Add another button? [y/N]:${RESET} " _more || true
    [[ "${_more:-}" =~ ^([yY]|yes)$ ]] || break
  done
fi
...
printf "\n${C_YELLOW}┌──────────────────────────────────────────────────────┐${RESET}\n"
printf "${C_YELLOW}│${RESET} ${C_CYAN}📡 Selected Protocol: VLESS WS${RESET}                         ${C_YELLOW}│${RESET}\n"
printf "${C_YELLOW}└──────────────────────────────────────────────────────┘${RESET}\n\n"

PROTO="vless-ws"
IMAGE="docker.io/nkka404/vless-ws:latest"

show_success "Protocol: ${C_CYAN}VLESS WebSocket${RESET}"
show_info "Docker Image: ${C_GRAY}$IMAGE${RESET}"
...
echo "  1) ${C_BLUE}🇸🇬 Singapore${RESET} (asia-southeast1) - ${C_GREEN}Recommended${RESET}"
echo "  2) ${C_BLUE}🇺🇸 United States${RESET} (us-central1)"
...
SERVICE="${SERVICE:-pussy1990-vless}"
...
printf "\n${C_YELLOW}┌──────────────────────────────────────────────────────┐${RESET}\n"
printf "${C_YELLOW}│${RESET} ${C_CYAN}🕒 Deployment Time${RESET}                                    ${C_YELLOW}│${RESET}\n"
printf "${C_YELLOW}└──────────────────────────────────────────────────────┘${RESET}\n\n"
...
show_kv "Image:" "${C_GRAY}docker.io/nkka404/vless-ws:latest${RESET}"
printf "\n"
...
URI="vless://${VLESS_UUID}@vpn.googleapis.com:443?path=%2F%40pussy1990&security=tls&encryption=none&host=${CANONICAL_HOST}&type=ws&sni=vpn.googleapis.com#Pussy1990-VLESS-WS"
...
# =================== QR Code Display ===================
printf "\n${C_GREEN}${BOLD}📱 QR CODE (Scan with V2Ray client):${RESET}\n"
...
━━━━━━━━━━━━━━━━━━━━━━━━━━
<blockquote>🌍 <b>Region:</b> ${REGION} 📡 <b>Protocol:</b> VLESS WebSocket 🔗 <b>Endpoint:</b> <a href="${URL_CANONICAL}">${URL_CANONICAL}</a> ⚙️ <b>Resources:</b> ${CPU} vCPU / ${MEMORY}</blockquote>
🔑 <b>VLESS Configuration:</b> <pre><code>${URI}</code></pre>
<blockquote>🕒 <b>Deployed:</b> ${START_LOCAL} ⏳ <b>Expires:</b> ${END_LOCAL}</blockquote>
━━━━━━━━━━━━━━━━━━━━━━━━━━
<b>Powered by @Pussy1990</b>
EOF
)
tg_send "${MSG}"
...
echo "  ${C_GRAY}•${RESET} Service is configured with ${C_GREEN}warm instances${RESET} (min-instances=1)"
...
show_divider
printf "\n${C_RED}${BOLD}@Pussy1990${RESET} ${C_GRAY}|${RESET} ${C_CYAN}VLESS WebSocket Deployment System${RESET} ${C_GRAY}|${RESET} ${C_GREEN}v2.0${RESET}\n"
printf "${C_GRAY}──────────────────────────────────────────────────────────${RESET}\n\n"
